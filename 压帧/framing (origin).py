...
def event2frame_SBT_txyp(event_stream_us, stack_duration_us, width, height, frame_per_stack=1):
    # preprocessing
    event_stream_us[:, 0] -= event_stream_us[0, 0]      #计算各个事件离第一个事件的时间
    event_stream_us[:, 3][event_stream_us[:, 3] == 0] = -1      #事件变为-1，1
...
if __name__ == '__main__':
    ...
    parser.add_argument('-input_dir', type=str, default='D:\\PytorchPro\\Data\\Gen1\\train_a\\detection_dataset_duration_60s_ratio_1.0\\train\\17-03-30_12-53-58_183500000_243500000_td.dat')
    parser.add_argument('-output_dir', type=str, default='./framed_data/gen1')
    parser.add_argument('-result_name', type=str, default='hello')
    args = parser.parse_args()

    result_npy = ''.join([args.result_name, '.npy'])
    result_gif = ''.join([args.result_name, '.gif'])

    ''' 1. extract the numpy integer ``txyp`` vectors (microsecond) '''
    ...
    npy_name = os.path.join(args.output_dir, result_npy)
    ...
    image_list = show_sign_frames_cv2(args.output_dir, frame_list)
    # generate the gif
    gif_name = os.path.join(args.output_dir, result_gif)
