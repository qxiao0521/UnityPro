...
from utils.criterion import build_criterion
from utils.misc import detection_collate, dvs_detection_collate, ori_target_frame_collate
...
def parse_args():
    ...
    # model
    parser.add_argument('-v', '--version', default='yolov2_tiny_bnn',
    ...
    parser.add_argument('--net_arch_fea', default='D:/PytorchPro/Data/Gen1/train_a/feature_network_path.npy', type=str)
    parser.add_argument('--cell_arch_fea', default='D:/PytorchPro/Data/Gen1/train_a/feature_genotype.npy', type=str)
...
#     log_path = "/media/HDD2/personal_files/liyanchen/PyTorch_YOLO-Family_logs/logs/retrain"
...
def train():
    ...
    # set distributed
    local_rank = 0
    if args.distributed:        # 没写多卡，这段代码没有用
    ...
    model_name = args.version   #‘yolov2_tiny_bnn’
    ...
    if model_name == 'yolov2_tiny_bnn':
        ...
        from models.yolo.yolov2_tiny_bnn import YOLOv2Tiny_BNN as yolo_net
    # from models.yolo.yolov2_tiny_bnn import YOLO_SNN as yolo_net 
    ...
    num_classes = 2
    # train_dataset = Gen1('/media/SSD5/personal/zhanghu', object_classes='all', height=240, width=304, augmentation=False, mode='train', 
    #             ms_per_frame = args.time_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, shuffle=False, transform=Resize_frame(train_size))
    # val_dataset = Gen1('/media/SSD5/personal/zhanghu', object_classes='all', height=240, width=304, augmentation=False, mode='val', 
    #             ms_per_frame = args.time_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, shuffle=False, transform=Resize_frame(val_size))
    train_dataset = Gen1_sbt('D:/PytorchPro/Data/Gen1/train_a_NoReshape', object_classes='all', height=240, width=304, mode='train',
                ms_per_frame = args.time_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, transform=Resize_frame(train_size), sbt_method='before')
    val_dataset = Gen1_sbt('D:/PytorchPro/Data/Gen1/val_a_NoReshape', object_classes='all', height=240, width=304, mode='val',
                ms_per_frame = args.time_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, transform=Resize_frame(val_size), sbt_method='before')
    # train_dataset = Gen1_sbn('/media/SSD5/personal/zhanghu/gen1', object_classes='all', height=240, width=304, mode='train', 
    #             events_per_frame = args.events_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, transform=Resize_frame(train_size), sbn_method='before')
    # val_dataset = Gen1_sbn('/media/SSD5/personal/zhanghu/gen1', object_classes='all', height=240, width=304, mode='val', 
    #             events_per_frame = args.events_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, transform=Resize_frame(val_size), sbn_method='before')
    # train_dataset = Gen1_sbn('/media/HDD3/personal_files/zhanghu/gen1/', object_classes='all', height=240, width=304, mode='train', 
    #             events_per_frame = args.events_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, transform=Resize_frame(train_size), sbn_method='before')
    # val_dataset = Gen1_sbn('/media/HDD3/personal_files/zhanghu/gen1/', object_classes='all', height=240, width=304, mode='val', 
    #             events_per_frame = args.events_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, transform=Resize_frame(val_size), sbn_method='before')
    # train_dataset = Gen1_sbt('/media/HDD3/personal_files/zhanghu/gen1/', object_classes='all', height=240, width=304, mode='train', 
    #             events_per_frame = args.events_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, transform=Resize_frame(train_size), sbn_method='before')
    # val_dataset = Gen1_sbt('/media/HDD3/personal_files/zhanghu/gen1/', object_classes='all', height=240, width=304, mode='val', 
    #             events_per_frame = args.events_per_frame, frame_per_sequence=args.frame_per_stack, T = args.time_steps, transform=Resize_frame(val_size), sbn_method='before')
    ...
    #                     pin_memory=True,
    #                     sampler=torch.utils.data.SubsetRandomSampler(train_indices[:54278])
    #                     )
    ...
    val_dataloader = torch.utils.data.DataLoader(
                        ...
                        num_workers=args.num_workers,
                        pin_memory=True,
                        # sampler=torch.utils.data.SubsetRandomSampler(val_indices[:640])
                        )

    
    print('Training model on:', args.dataset)
    print('The dataset size:', len(train_dataset))#这里可以直接改成num_train
    print("----------------------------------------------------------")

    # build model
    anchor_size = cfg['anchor_size_gen1']#这里写死不好，应该改为anchor_size_'dataset_name' 呃呃，好像是没用，后面model会覆盖，应该是随便声明一下
    ...
    #                args=args)
    anchor_size = cfg['anchor_size_gen1']#重复了，删掉 呃呃，好像是没用
    net = yolo_net(device=device, 
                   input_size=train_size, 
                   num_classes=num_classes, 
                   trainable=True, 
    ...
    for epoch in range(args.start_epoch, max_epoch):
        ...
        for iter_i, (images, targets, original_label, original_frame, file) in enumerate(train_dataloader):
            ...
            # conf_pred, cls_pred, txtytwth_pred, x1y1x2y2_pred = model.stream_forward_new(images, file)
        ...
        # if True:
            ...
            with torch.no_grad():
                for id_, data in enumerate(val_dataloader):
                    # print('reset all mem')
                    # for key in mem_keys:
                    #     exec('model_eval.%s.mem=None'%key)
                    ...
                    # conf_pred, cls_pred, reg_pred, box_pred = model.stream_forward_new(images, file)
                    # conf_pred, cls_pred, txtytwth_pred, x1y1x2y2_pred, bias_loss = model_eval(image)
                    
                    # conf_out_of_range += len(conf_pred[conf_pred<0])
                    # conf_out_of_range += len(conf_pred[conf_pred>1])
                    # box_out_of_range += len(x1y1x2y2_pred[x1y1x2y2_pred<0])
                    # box_out_of_range += len(x1y1x2y2_pred[x1y1x2y2_pred>1])
                    ...
                    bboxes = [tools.resized_box_to_original(box, val_size, 240, 304) for box in bboxes]
            ...
            # bias_loss_item = sum(bias_loss_list).item() / batch_num
            ...
            # print('val/bias loss',  bias_loss_item)
            ...
            fitlog.add_metric(name="Valid mAP(0.5)",value=map50,step=epoch)
            fitlog.add_metric(name="Valid mAP(0.5:0.95)",value=map50_95,step=epoch)
            fitlog.add_loss(name="Total loss",value=total_loss_item,step=epoch)

            if cur_map > best_map:
                
                # update best-map
                best_map = cur_map
                fitlog.add_best_metric(name="Valid Best mAP(0.5)",value=best_map)
