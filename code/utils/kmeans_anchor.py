...
if __name__ == "__main__":
    ...
    # dataset_coco = COCODataset(data_dir=os.path.join(args.root, 'coco2017'),
    #                             img_size=img_size)
    ...
    #             continue
    #         boxes.append(Box(0, 0, bw, bh))
    dataset_gen1 = Gen1('/media/SSD5/personal/zhanghu', object_classes='all', height=240, width=304, augmentation=False, mode='train', 
                ms_per_frame = 10, frame_per_sequence=5, T = 5, shuffle=False, transform=None)
    labels = dataset_gen1.labels
    r = img_size / 304
    labels[:, 3:5] = labels[:, 3:5]  * r
    boxes = []
    for label in labels:
        boxes.append(Box(0, 0, label[3], label[4]))
