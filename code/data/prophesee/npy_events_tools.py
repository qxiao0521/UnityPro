...
def parse_header(fhandle):
    ...
    ev_size = dtype.itemsize    #读出数据类型的存储大小（每个元素所占的字节数）
    assert ev_size != 0
    start = fhandle.tell()  #回到读写指针位置
    # turn numpy.dtype into an iterable list
    ev_type = [(x, str(dtype.fields[x][0])) for x in dtype.names]   #把类型名变成一个（数据名，数据类型）迭代器
    # filter name to have only t and not ts
    ev_type = [(name if name != "ts" else "t", desc) for name, desc in ev_type]     #如果有数据名为ts就把它改成t
