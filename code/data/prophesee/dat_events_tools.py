...
def count_events(filename):
    ...
    """
    with open(filename, 'rb') as f:
        bod, _, ev_size, _ = parse_header(f)    # 一个数据大小是8个字节
        f.seek(0, os.SEEK_END)  # 文件尾部位置
        eod = f.tell()
...
def parse_header(f):
    ...
    """
    f.seek(0, os.SEEK_SET)  # 将文件的读取指针移动到文件开头
    bod = None  # 头部结束后的文件位置，即数据存储的开始位置
    end_of_header = False   # 是否达到文件头部的末尾，一个flag
    header = [] # 用于存储头部包含的信息的list
    num_comment_line = 0    # 存储头部注释行数  干嘛用的？
    size = [None, None] # 数据尺寸
    # parse header
    while not end_of_header:
        bod = f.tell()  # f.tell() 获得但钱文件指针的位置
        line = f.readline() # 读取下一行
        if sys.version_info > (3, 0):
            first_item = line.decode("latin-1")[:2] # 根据python版本解码读取的行并获取前两个字符
        else:
            first_item = line[:2]
        # 判断读取这两个字符是不是‘% ’，不是则标记头部结束，是则处理
        if first_item != '% ':
            end_of_header = True
        else:
            words = line.split()    #分割成单词
            if len(words) > 1:
                if words[1] == 'Date':
            ...
            num_comment_line += 1
    # parse data
    f.seek(bod, os.SEEK_SET)    #文件指针回到头部结束的位置
